...
//我们没有真正解析HTTP请求的消息体，只是判断它是否被完整得读入了
...
bool http_conn::write()
{
    ...
    while(1)
    {
        ...
        if( temp <= -1)
        {
            //如果TCP写缓冲没有空间，则等待下一轮EPOLLOUT事件。虽在此期间，服务
        //器无法立即接受到同一客户的下一个请求，但是这可以保证链接的完整性
            if(errno == EAGAIN)
            {
        ...
        {
            //发送HTTP相应成功，根据HTTP请求中的Connection字段决定是否立即关闭链接
            unmap();
            if( m_linger)
            {
                init();
